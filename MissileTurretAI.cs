...
public class MissileTurretAI : NetworkBehaviour
{
    ...
    private void Update()
    {
        ...
        switch (_state)
        {
            case MissileTurretState.SEARCHING:
                ...
                if (Physics.Raycast(rod.position + rod.up, rod.up, out RaycastHit hit, 30f, 1051400,
                        QueryTriggerInteraction.Ignore) && hit.transform.CompareTag("Player"))
                {
                    ...
                    // This check prevents it from entering the CHARGING state if _targetPlayer is null, preventing the CHARGING -> SEARCHING loop
            ...
            case MissileTurretState.CHARGING:
                ...
                {
                    _currentChargeTime = ChargeTimeSeconds;
                    if ((!Physics.Raycast(rod.position + rod.up, _targetPlayer.transform.position - rod.position,
                    ...
                    else stateToChangeTo = MissileTurretState.FIRING;
    ...
    {
        laser.SetActive(active); // it was necessary, I checked
    }
